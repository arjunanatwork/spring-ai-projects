package org.personal.springai.rag;
...
public class WebSearchDocumentRetriever implements DocumentRetriever {
    ...
    public WebSearchDocumentRetriever(RestClient.Builder clientBuilder, int resultLimit) {
        ...
        Assert.hasText(apiKey, "TAVILY_SEARCH_API_KEY environment variable must be set");
        this.restClient = clientBuilder.baseUrl(TAVILY_BASE_URL).defaultHeader(HttpHeaders.AUTHORIZATION, apiKey).build();
        if(resultLimit <= 0) {
